  ...
  callbacks: {
    async signIn({ user, account, profile }) {
      if (account?.provider === "google") {
        // Trigger handle_new_user is automatic when Supabase Adapter creates the user
        // But we can ensure metadata is passed if needed
        return true;
      }
      return true;
    },
    ...
    async jwt({ token, user, trigger, session }) {
      ...
      // Fetch latest role and profile status from DB
      if (token.id) {
